...
const fGroupedConcerts = (concerts) => {
  try {
    ...
    const grouped = Object.groupBy(
      concerts,
      (concert) => concert.artist || "Sin Artista"
    );
...
const fetchConcerts = async () => {
  loading.value = true;
  try {
    const response = await ConcertService.getConcertsByArtist();
    concerts.value = Array.isArray(response)
      ? response
      : Object.values(response);
    artists.value = [...new Set(concerts.value.map((concert) => concert.artist))]
      .sort()
  ...
  if (selectedArtist.value) {
    groupedConcerts.value = {
      [selectedArtist.value.name]:
        groupedConcerts.value[selectedArtist.value.name],
    };
  }
...
<template>
  <div class="concert-list">
    ...
    <div>
      <div
        v-for="[artist, artistConcerts] of Object.entries(groupedConcerts)"
        :key="artist"
        class="card-section"
      >
        <h3>{{ artist }}</h3>
        <div class="cards-grid">
          <Card
            v-for="concert in artistConcerts"
            :key="concert.id"
            :class="
              artistConcerts.length === 1
                ? 'single-concert'
                : artistConcerts.length === 2
                ? 'double-concert'
                : 'concert-card'
            "
          >
